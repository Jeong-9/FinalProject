# ChemiBot - 비접촉 화학 실험 자동화 및 안전 관제 시스템
...
## 주요 기능
- 손동작 인식을 통한 로봇팔 제어.
- 얼굴 방향 추적을 통한 화면 내 시험관대 선택.
- 가스, 온도, 습도 기반 실시간 환경 모니터링.
- 실험실 및 보안실 모니터링 WPF 프로그램.
- React 기반 관리자 웹 화면.
- 스위치, 가스누출, 쓰러짐 감지를 통한 비상정지 기능.
...
- 실험실 아크릴 박스 3D 모델링 및 2D 제작 도면을 작성.
- 아크릴판 재단 및 실험실 아크릴 박스를 제작.
- Raspberry Pi 기반 내부문 및 잠금장치 서보모터 제어 코드를 작성.
- C# WPF 기반 보안실 모니터링 화면을 구현.
- TCP를 기반으로 실험실 웹캠 영상을 수신 받아 보안실 화면에 띄우고, UDP 기반의 비상 신호를 처리.
- UDP를 기반으로 보안실 마이크 음성을 송신해서 실험실 스피커에 출력하는 기능 구현.
...
| `Servo/door_server.py` | TCP 기반 서보모터 제어 명령 수신 |
...
- Solid Edge를 사용해 실험실 아크릴 박스의 전체 구조를 3D 모델링했습니다. 로봇팔 동작 범위와 실험 도구 배치 공간을 고려해 각 판의 크기를 설계했습니다.
## 2. 2D 설계도면 작성 
...
- 완성된 도면을 기준으로 아크릴판을 재단하고 실제 실험 공간을 제작했습니다. 여기서 내부 유해 물질이 외부로 빠져나가는 것을 방지하기 위해 외부 문과 내부 문으로 구성된 이중문 구조로 설계하고 제작했습니다.
...
- `door_server.py`는 TCP를 통해 명령을 수신하고, `door_control.py`의 서보 동작 함수와 연결합니다.
- 실험 시작, 내부문 열림·닫힘, 잠금 해제·복귀 명령을 구분하여 실행하도록 구현했습니다.
- 내부문 열림 명령이 들어올 때 잠금장치가 잠기지 않은 상태라면, 먼저 잠금장치를 잠근 뒤 내부문을 열도록 구성했습니다. 이를 통해 실험 공간이 외부에 직접 노출되지 않도록 했습니다.
...
- 중앙 카메라 영역은 실험실 측 PC에서 전송한 웹캠 영상을 TCP로 수신하여 실시간으로 출력합니다.
...
- 비상상황 발생 시 보안실 PC와 연결된 마이크를 통해 안내방송을 할 수 있습니다. 이 음성은 보안실에서 실험실 스피커로 전달되도록 구현했습니다.
- 비상 신호는 UDP로 수신하며, GAS_SENSOR, EME_BUTTON, FALL_DOWN 값을 통해 가스 감지, 비상 버튼, 쓰러짐 감지를 구분합니다.
- 실험실의 lab_intercom.py는 UDP로 수신한 보안실 음성을 실험실 스피커로 출력하도록 구현했습니다.
- 비상상황이 종료되면, 보안실 마이크 음성 수신도 종료되도록 구현했습니다.
