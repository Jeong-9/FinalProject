...
- 실험실 아크릴 박스 3D 모델링 및 2D 제작 도면 작성.
- 아크릴판 재단 및 이중문 구조 조립.
- Raspberry Pi 기반 내부문 및 잠금장치 서보모터 제어 코드 작성.
- TCP 기반 서보모터 제어 서버 구현.
- C# WPF 기반 보안실 모니터링 화면 구현.
- TCP 기반 영상 수신 및 UDP 기반 비상 신호 처리.
- UDP 기반 보안실 마이크 음성 송신 및 실험실 스피커 출력 기능 구현.
...
- Solid Edge를 사용해 실험실 아크릴 박스의 전체 구조를 3D 모델링했습니다. 로봇팔 동작 범위와 실험 도구 배치 공간을 고려해 내부 크기와 이중문 구조를 설계했습니다.
## 2. 2D 설계도면 작성 
...
- 실험실의 lab_intercom.py는 UDP로 수신한 보안실 음성을 실험실 스피커로 출력합니다.
